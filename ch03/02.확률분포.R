...
# 이항분포를 따르는 n개의 표본 추출
set.seed(2023)
rbinom(10, n, p)            # 2 1 1 2 0 1 2 2 1 2
...
sigma = 6
# (180 - mu) / 6
ll = mu - 3 * sigma         # 152, lower limit
ul = mu + 3 * sigma         # 188, upper limit
x = seq(ll, ul, by=0.01)
...
# 95% 에 속하는 학생들의 키 범위는?
ll = round(qnorm(0.025, 170, 6), 2)         # 158.24
ul = round(qnorm(0.975, 170, 6), 2)         # 181.76
print(paste(ll, '~', ul, '(cm)'))

# 400개의 난수를 생성하여 모집단과 비교
set.seed(2023)
smp = rnorm(400, 170, 6)
print(paste(mean(smp), sd(smp)))            # 170.187716263964 5.92263438665975
hist(smp, prob=T, main='N(170, 6^2)으로부터 추출한 표본의 분포(n=400)',
     xlab='', ylab='', col='white', border='black')
lines(x, px, lty=2, lwd=2, col='red')       # lty=2 : 점선
...
p0.05                   # 1.645, 90% 신뢰수준
p0.025 = qnorm(0.975)
p0.025                  # 1.960, 95% 신뢰수준
pnorm(1.96) - pnorm(-1.96)      # 0.95
qnorm(0.995)            # 2.576, 99% 신뢰수준
